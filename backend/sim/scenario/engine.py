...
class ScenarioEngine:
    ...
    async def run(self, scenario: Scenario, *, session_id: Optional[str] = None) -> RunResult:
        ...
        raise NotImplementedError(f"service_type={scenario.service_type} 미구현 (1차는 MCPTT)")
    ...
    async def _run_mcptt(self, scenario: Scenario, session_id: str) -> RunResult:
        ...
        for spurt_exp in exp.talk_spurts:
            sr = await self._do_talk_spurt(scenario, session_id, call_id, spurt_exp, ft)
    ...
    async def _do_talk_spurt(self, scenario: Scenario, session_id: str, call_id: str,
                             spurt_exp, ft: int) -> SpurtResult:
